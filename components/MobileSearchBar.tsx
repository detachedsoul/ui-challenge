const MobileSearchBar = ({isActive}: {isActive: boolean}): JSX.Element => {
    return (
        <div className={`bg-white p-4 rounded-[0.875rem] border border-[#edf2f7] absolute top-[calc(100%+1rem)] w-[calc(100%-2rem)] transition-transform ease-in-out duration-300 ${isActive ? "translate-y-0" : "-translate-y-[200%]"} lg:hidden`}>
            <form className="flex items-center bg-white border border-[#dadddd] px-4 py-3 gap-2 rounded-3xl" method="get">
                ...
                <input className="text-[#a3a3a3] leading-6 focus:outline-none focus:ring-0 focus:border-none bg-transparent w-full" type="search" name="search" id="search" placeholder="Search..." />
            </form>
        </div>
    );
};
